...
$.fn.getGridHeight = function (hasPage) {
    var pageHeight = 25;//一种奇怪的bug，临时解决方案-18px
    if (hasPage) {
...
$.fn.ComboBox = function (options) {
    ...
    if (options.url != undefined) {
        ...
        $.ajax({
            ...
            statusCode: {
                401: function (data) {
                    //401为服务器返回的自定义状态码，说明当前操作没有权限
                    $.fn.modalAlert("您没有权限进行此项操作，请重新登陆！", "warning");
                }
            },
...
$.fn.jqGridIndex = function (column, data) {
    var $jgrid = $(this);
    var index = 0;
    var rows = $jgrid.jqGrid("getRowData");
    for (var i = 0; i < rows.length; i++) {
        if (rows[i][column] !== data) continue;
        index = i + 1;
        break;
    }
    return index;
}
$.fn.jqGridSetRowData = function (id, rowId, data) {
    var $jgrid = $(this);
    var rows = $jgrid.jqGrid("getRowData");
    for (var i = 0; i < rows.length; i++) {
        if (rows[i][name] !== rowId) continue;
        $jgrid.setRowData(i + 1, data);
        break;
    }
}
$.fn.bootStrapRows = function () {
    var $table = $(this);
    return $table.bootstrapTable('getSelections');
}
$.fn.bootStrapSelections = function (column) {
    var $table = $(this);
    var json = [];
    var rows = $table.bootstrapTable('getSelections');
    for (var i = 0; i < rows.length; i++) {
        json.push(rows[i][column]);
    }
    return json.join(",");
}
$.fn.modalOpen = function (options) {
    ...
    top.layer.open({
        ...
        yes: function (index) {
            options.callBack("layui-layer-iframe" + index) == undefined;
        ...
        success: function (layero, index) {
            var router = layero.context.referrer;
            //console.log(router);
            //console.log($(layero.selector));
            if (router == null || router == undefined) return;
            //获取URL路由，如果路由为登陆页面则在顶层页面跳转到登陆页面
            //top.location.href = router + '?returnurl=' + options.url;
        }
    });
}
//$.fn.isJSON = function (data) {
...
$.fn.submitFileForm = function (options) {
    var defaults = {
        url: "",
        param: [],
        loading: "正在提交数据...",
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success == true) {
                    ...
                    if ((data.url == null || data.url == undefined) == false) {
...
$.fn.submitAjax = function (options) {
    ...
    window.setTimeout(function () {
        $.ajax({
            url: options.url,
            data: options.param,
            type: options.type == undefined ? "POST" : "GET",
            dataType: "json",
            traditional: true,
            statusCode: {
                401: function (data) {
